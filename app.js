...
function validarNombre(nombre) {
    ...
    if (nombre.length < 3 || nombre.length > 150 || nombre.split(' ').length < 2 || nombre == '' || nombre.match(/[0-9]/)) {
        resultado = false;
    }

    let tamañoNombre = nombre.split(' ').filter(nom => nom.length <= 1)
    if (tamañoNombre.length > 0) resultado = false
